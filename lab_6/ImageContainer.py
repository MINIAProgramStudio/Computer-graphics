...
def IC_to_monochrome(input_IC, red = 0.36, green = 0.53, blue = 0.11): #create monochromatic copy of an image
    ...
    for row in output_IC.data:
        pos_x = 0
        for pixel in row:
            output_IC.data[pos_y][pos_x] = int(pixel[0]*red+pixel[1]*green+pixel[2]*blue) #monochromatic transformation
            pos_x+=1
        pos_y+=1
...
def built_in_image(input_IC, binary_IC, channel = 2, bit = 1):
    ...
    for row in input_IC.data:
        ...
        for pixel in row:
            ...
            bitted_value = unbited_value + int(binary_IC.data[pos_y%binary_height][pos_x%binary_width][0]/255)*(2**(bit-1))
